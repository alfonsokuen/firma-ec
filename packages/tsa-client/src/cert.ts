 ...
 */
function extractCN(rdns: { typesAndValues: { type: string; value: { valueBlock: { value?: string } } }[] }[]): string | null {
...
export function parseCertFromDer(der: Uint8Array): ParsedCert {
  ...
  const issuerRdns = cert.issuer.typesAndValues
    ? [{ typesAndValues: cert.issuer.typesAndValues as unknown as { type: string; value: { valueBlock: { value?: string } } }[] }]
    : [];
  return {
