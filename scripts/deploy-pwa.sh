...
ssh root@$HOST "cd /root/firma-ec-build && docker build --build-arg VITE_HANDOFF_ALLOWLIST='${VITE_HANDOFF_ALLOWLIST:-}' --build-arg VITE_STORE_URL='${VITE_STORE_URL:-https://tienda.firmar.ec}' -f infra/docker/pwa.Dockerfile -t $IMAGE ."
