...
### Added
- **Guarda hermética `check-nav-links` en el build de la landing** (`@firma-ec/landing` 0.7.3). Corre dentro de `pnpm build`, así que la ejercita el CI, y fija las dos decisiones anteriores sobre el `dist`: el menú apunta a la herramienta (2 anclas por lengua, escritorio y móvil) y el artículo conserva enlace desde el pie. Probada **en las dos direcciones**: enrojece si el menú vuelve al artículo y si el pie pierde el enlace. El canary Playwright equivalente habla con producción y por eso no puede ser gate de PR.
