...
/** Pre-configured bucket specs from spec §4. */
export const BUCKETS = {
  /** 1 message per 30s per phone. */
  msgPerPhone: (phoneHash: string): BucketSpec => ({
    key: `rl:msg:${phoneHash}`,
    capacity: 1,
    refillPerSec: 1 / 30,
  }),
  /** 5 PDFs per day per phone. */
  pdfPerDay: (phoneHash: string): BucketSpec => ({
    key: `rl:pdf:${phoneHash}`,
    capacity: 5,
    refillPerSec: 5 / 86_400,
  }),
  /** 10 OTP attempts per hour per IP. */
  otpPerIp: (ip: string): BucketSpec => ({
    key: `rl:otp-ip:${ip}`,
    capacity: 10,
