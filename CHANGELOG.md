...
- **Verificar: mensaje claro para PDF truncado/incompleto en vez de error genérico** (`@firma-ec/pwa` 0.9.13): un `/ByteRange` que apunta más allá del fin del archivo (`c+d > fileSize`) significa que el contenido firmado ya **no está completo** — típico de una **descarga interrumpida** o un archivo **truncado al copiarlo** (caso real: copias parciales de `S00071_firmada_*.pdf`, a las que les faltaban ~1.5 KB finales). Antes el código `byterange_invalid` caía en el mensaje genérico "error inesperado". Ahora se mapea a un mensaje dedicado (ES+EN) que explica que el documento parece incompleto/dañado y sugiere **volver a descargar el PDF original**. El veredicto sigue siendo `inválido` (correcto: no se puede verificar contenido ausente).
