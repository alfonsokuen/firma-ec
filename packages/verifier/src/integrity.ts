...
import { VerificationError, ERR_WEAK_HASH, ERR_WEAK_SIG, ERR_RSA_TOO_SMALL } from './errors';
...
function buildCoveredBytes(pdfBytes: Uint8Array, byteRange: [number, number, number, number]): Uint8Array {
...
export async function checkDocumentIntegrity(
  ...
  let same = true;
  for (let i = 0; i < computed.length; i++) if (computed[i] !== expectedDigest[i]) { same = false; break; }
...
export async function verifySignatureValue(
  ...
  if (REJECTED_SIG_OIDS.has(signatureAlgoOid)) {
    throw new VerificationError(ERR_WEAK_SIG, `Rejected weak signature algorithm OID ${signatureAlgoOid}`);
  }

  // Determine algorithm name + hash for Web Crypto importKey/verify
  const hashAlgo = HASH_OID_TO_ALGO[digestAlgoOid];
  if (!hashAlgo) throw new VerificationError(ERR_WEAK_HASH, `Unsupported hash OID for signature: ${digestAlgoOid}`);

  const isRsa = signatureAlgoOid === '1.2.840.113549.1.1.11' || signatureAlgoOid === '1.2.840.113549.1.1.12' || signatureAlgoOid === '1.2.840.113549.1.1.13' || signatureAlgoOid === '1.2.840.113549.1.1.1';
  const isRsaPss = signatureAlgoOid === '1.2.840.113549.1.1.10';
  const isEcdsa = signatureAlgoOid === '1.2.840.10045.4.3.2' || signatureAlgoOid === '1.2.840.10045.4.3.3';
  ...
  const spkiDer: ArrayBuffer = spkiRaw.buffer.slice(spkiRaw.byteOffset, spkiRaw.byteOffset + spkiRaw.byteLength) as ArrayBuffer;
  ...
  if (isRsa) {
    const pubkey = await crypto.subtle.importKey('spki', spkiDer, { name: 'RSASSA-PKCS1-v1_5', hash: hashAlgo }, false, ['verify']);
    // RSA-2048 minimum (modulus length in bits)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-member-access
    const modulusHex = (signerCert.subjectPublicKeyInfo.parsedKey as any)?.modulus?.valueBlock?.valueHex as ArrayBuffer | undefined;
    const modulusBits = modulusHex ? modulusHex.byteLength * 8 : undefined;
    if (modulusBits && modulusBits < 2048) {
      throw new VerificationError(ERR_RSA_TOO_SMALL, `RSA modulus too small: ${modulusBits} bits (need ≥2048)`);
    }
    return await crypto.subtle.verify('RSASSA-PKCS1-v1_5', pubkey, toArrayBuffer(signatureValue), toArrayBuffer(signedAttrsDer));
  }

  if (isRsaPss) {
    // saltLength typically equals digest length for PSS-with-MGF1
    const saltLength = hashAlgo === 'SHA-256' ? 32 : hashAlgo === 'SHA-384' ? 48 : 64;
    const pubkey = await crypto.subtle.importKey('spki', spkiDer, { name: 'RSA-PSS', hash: hashAlgo }, false, ['verify']);
    return await crypto.subtle.verify({ name: 'RSA-PSS', saltLength }, pubkey, toArrayBuffer(signatureValue), toArrayBuffer(signedAttrsDer));
  }

  if (isEcdsa) {
    const namedCurve = hashAlgo === 'SHA-256' ? 'P-256' : 'P-384';
    const pubkey = await crypto.subtle.importKey('spki', spkiDer, { name: 'ECDSA', namedCurve }, false, ['verify']);
    ...
    return await crypto.subtle.verify({ name: 'ECDSA', hash: hashAlgo }, pubkey, toArrayBuffer(rsBytes), toArrayBuffer(signedAttrsDer));
  }

  throw new VerificationError(ERR_WEAK_SIG, `Unsupported signature algorithm OID ${signatureAlgoOid}`);
}

async function asn1ToRawEcdsa(asn1Sig: Uint8Array, fieldBytes: number): Promise<Uint8Array> {
  const { fromBER } = await import('asn1js');
  const parsed = fromBER(asn1Sig.buffer.slice(asn1Sig.byteOffset, asn1Sig.byteOffset + asn1Sig.byteLength) as ArrayBuffer);
