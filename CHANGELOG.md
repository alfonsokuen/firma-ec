...
### Added
- **El motor ya sabe donde TERMINA cada linea, no solo donde empieza** (`@firma-ec/signer` 0.11.0). `LineStart` gana `end`: la suma de los anchos de glifo de la fuente vigente —`/Widths` + `/FirstChar` + `/MissingWidth` en fuentes simples, `/W` + `/DW` en Type0/CID, y una tabla AFM de las 14 estandar (`standardFontWidths.ts`, extraida de `@pdf-lib/standard-fonts`) para las que no incrustan metricas— con `Tc`, `Tw`, `Tz` y los ajustes de `TJ`, ya transformada por la matriz efectiva. La matriz de texto del recorredor **no** se toca: moverla cambiaria los arranques que validan las tablas congeladas, asi que el borde derecho se anota aparte. Contrastado contra pymupdf en las 8 paginas de bloque de firma del corpus real (448 lineas): **mediana 0,00 pt, p90 2,52 pt, maximo 7,53 pt** — dentro del criterio con el que se acepto (mediana ≤ 3, maximo ≤ 8).

### Changed
- **La estampa automatica va CENTRADA sobre el nombre/raya del firmante, no apoyada en el margen izquierdo del bloque** (`@firma-ec/pwa` 0.24.0). Una persona firma **sobre** el nombre; el motor lo apoyaba a su izquierda. Medido en produccion (0.23.4) sobre los 8 documentos reales, la caja quedaba entre **+35 y +88 pt** a la derecha del centro del nombre: una carta con el firmante centrado en x=313 recibia la caja en x=281,7 con 240 de ancho, o sea centrada en 401,7. El centro se calcula como el punto medio de la union `[min x, max end]` de los arranques de la **primera** columna del bloque (la misma que ya delimita `columnSplit`), y se aplica en los DOS caminos que anclaban al bloque: el documento sin firmas (`placeOnLastPage`) y el que ya trae una (anti-solape), que es justamente el de la firma individual en produccion.
- **Efecto medido en el corpus real (centro de la caja contra el centro real del bloque, `pymupdf get_text('words')`)**: los 8 documentos quedan dentro de **0,10 pt** del centro real (antes: 35–88 pt de desvio). Ninguno pisa texto ni la firma previa, y en los 6 con cofirmante el borde derecho sigue por debajo de su columna. Las dos entradas del corpus congelado se mueven con su medida: `contrato2026` x 63,9 → 18,0 y w 204,7 → 212,6 (centro real 124,3, la caja pasa de 166,3 a 124,3); `lideres` x 140,3 → 57,3 y w 150,6 → **233,6** — ahi el centrado ademas **recupera ancho**, porque la caja deja de crecer hacia la columna del cofirmante y ya no hace falta recortarla.
- **Si la caja centrada no cabe, se encoge SIMETRICA**, nunca se desplaza: mover el centro para ganar ancho seria deshacer lo unico que este cambio hace. Por debajo del suelo legible del layout (78 pt, el que evita firmar una estampa muda) se abandona el centrado entero y manda el comportamiento anterior. Un `boxW` explicito —el hint de propagacion del lote— sigue intacto: es una decision de quien firma y el motor no la pisa.

### Notas
- Sin borde derecho estimado en ninguna linea del bloque (fuente sin `/Widths` y sin metricas estandar) el motor **no opina** sobre el centro y se coloca como antes. En el corpus real eso no ocurre: las 8 paginas de bloque de firma tienen `end` en el 100% de sus arranques.
- **Coste conocido, congelado en los tests**: en un bloque de etiqueta/valor a dos columnas ("Nombre:" a la izquierda, el nombre a la derecha) la primera columna son las ETIQUETAS, asi que la caja se centra sobre la etiqueta y se estrecha. Antes tampoco cubria el nombre —salia de 62,9 a 302,9 con el valor arrancando en 340—, asi que el centrado no crea el defecto: lo estrecha. El motor no distingue etiqueta de valor desde los arranques de linea, la misma limitacion que ya documenta `columnSplit`.
- **Bloques pegados al margen izquierdo acaban con la caja en el margen y mas estrecha** (p.ej. un bloque cuyo centro cae a 101 pt del borde deja la caja en x=18 con 202 pt en vez de 240). Es consecuencia directa de encoger simetrico, y pasa en 2 de los 8 documentos reales. Verificado que ninguno pisa nada.
- 22 tests nuevos (12 del estimador de fin de linea, 7 del centrado, y 3 mas de guarda) y **12 mutaciones probadas, todas cazadas**: quitar el gate `widthIsDefault` en cada uno de los dos caminos, quitar el suelo de legibilidad, ignorar la frontera de columna al calcular el centro, desplazar en vez de encoger, aflojar la guarda de texto girado/espejado, contar un glifo sin ancho como cero, no reiniciar el avance en `Td`, aplicar `Tw` a fuentes de 2 bytes, no medir las cadenas en blanco, creerse una Type0 de longitud impar, y quedarse con el primer `end` en vez del mayor al deduplicar arranques.
