...
describe('requestTimestamp orchestration', () => {
  ...
  it('returns error:timeout when fetch is aborted', async () => {
    ...
    const r = await requestTimestamp(new Uint8Array(32), {
  ...
  it.skipIf(!HAS_KAT)(
    'returns malformed when fixture imprint mismatches request imprint',
    async () => {
      ...
      vi.stubGlobal(
        'fetch',
        vi.fn(async () => new Response(tsr, { status: 200 })),
      );
  ...
  it.skipIf(!HAS_KAT)(
    'returns TimestampOk when fixture matches captured imprint+nonce',
    async () => {
      // To bypass the random nonce, we have to stub crypto.getRandomValues to seed
      // the same nonce as in the fixture meta.
      const meta = JSON.parse(readFileSync(FIXTURE_META, 'utf-8')) as {
        imprintHex: string;
        nonceHex: string;
      };
